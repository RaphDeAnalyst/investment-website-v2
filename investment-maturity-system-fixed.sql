-- Investment Maturity Processing System (Fixed Version)
-- This SQL file creates all functions needed for the cron job to work properly
-- Fixes the return type conflict by dropping existing functions first
...
BEGIN
  ...
  WHERE i.status = 'active'
  AND i.maturity_date > CURRENT_DATE
  AND i.maturity_date <= CURRENT_DATE + INTERVAL '%s days'
  FORMAT(days_ahead)
  ORDER BY i.maturity_date ASC;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
